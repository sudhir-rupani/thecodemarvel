...
(function() {
  this.jKanban = function() {
	...
	element.class.forEach( function(cl){
      ...
      nodeItem.innerHTML = __buildItemTitle(element);
      //add function
    ...
    function __buildItemTitle(itemObj) {
      var result = itemObj;
      if (self.options.itemHandleOptions.enabled) {
        if ((self.options.itemHandleOptions.customHandler || undefined) === undefined) {
          ...
          result = "<div class='item_handle "+customCssHandler+"'><i class='item_handle "+customCssIconHandler+"'></i></div>"+
            "<div>"+( (result.title)?result.title:'')+"</div>"+
            "<div>" +( (result.date)?result.date:'') + "</div>"+
            "<div>"+( (result.status)?result.status:'') +"</div>";
        } else {
          result = self.options.itemHandleOptions.customHandler(result);
        }
      }
      return result;
    }
